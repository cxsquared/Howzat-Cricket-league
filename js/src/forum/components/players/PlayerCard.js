...
export default class PlayerCard extends Component {
    ...
    buildHeader(player) {
        const colorString = this.player.user().color() ? this.player.user().color() : '#675555';
        ...
        if (this.attrs.showUser && user && usernameSpan[0] !== '[deleted]') {
            userLink = [
                <div className="PlayerCard-username">
                    <a href={app.route('user', { username: user.username() })} style={{ color: textColor }}>
                        {usernameSpan}
                    </a>
                </div>,
                seperator,
        ...
        if (canEdit && !this.saving) {
            ...
            headerButtons.push(
                <Button onclick={this.toggleUpdating.bind(this)} icon="fas fa-cog" disabled={!hasTpe} className={className}>
                    {text}
                </Button>
            );
        }
                ...
                <div className="PlayerCard-info">
                    ...
                    <div className="PlayerCard-subheader">
                        ...
                        <div className="PlayerCard-data">
                            {`${app.translator.trans('hcl.forum.player.tpe')}: ${player.tpe()} (${app.translator.trans(
        ...
        return (
            <div className="PlayerCard-batter">
                ...
                <div className="PlayerCard-batter-attributes">
                    <div className="PlayerCard-skill-label">{app.translator.trans('hcl.forum.player.running')}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans('hcl.forum.player.defense')}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans('hcl.forum.player.attacking')}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans('hcl.forum.player.lofted')}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans('hcl.forum.player.vsSpin')}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans('hcl.forum.player.vsPace')}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans('hcl.forum.player.footwork')}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans('hcl.forum.player.timing')}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans('hcl.forum.player.control')}</div>
                    {battingStats}
                </div>
            </div>
        );
        ...
        return [
            ...
            <div className="PlayerCard-bowler">
                <div className="PlayerCard-attributes-header">
                    ...
                    <div className="PlayerCard-info">
                        {`${app.translator.trans('hcl.forum.player.bowling_style')}: ${app.translator.trans(
                            `hcl.forum.player.style.${style.toLowerCase()}`
                        )}`}
                    </div>
                </div>
                <div className="PlayerCard-bowler-attributes">
                    <div className="PlayerCard-skill-label">{app.translator.trans(`hcl.forum.player.paceFlight.${style}`)}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans(`hcl.forum.player.swingLegSpin.${style}`)}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans(`hcl.forum.player.slowerBallOffSpin.${style}`)}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans(`hcl.forum.player.seamDrift.${style}`)}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans(`hcl.forum.player.accuracy`)}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans(`hcl.forum.player.discipline`)}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans(`hcl.forum.player.bouncerBounce.${style}`)}</div>
                    <div className="PlayerCard-skill-label">{app.translator.trans(`hcl.forum.player.yorkerArmBall.${style}`)}</div>
                    {stats}
                </div>
            </div>,
        ];
