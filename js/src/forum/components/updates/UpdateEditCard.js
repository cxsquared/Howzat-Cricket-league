...
export default class UpdateEditCard extends Component {
    ...
    oncreate(vnode) {
        ...
        if (!this.loadingPlayer && !this.dpInit) {
            this.dpInit = true;
            this.datepicker = getDatepicker(`#Update-datepicker-${this.update.id()}`, this.date(), (instance, date) => this.date(date));
        }
    }
                ...
                <div className="darkenBackground UpdateEditCard-wrapper">
                    <div className="UpdateEditCard-fields">
                        ...
                        <div className="UpdateEditCard-item UpdateEditCard-week">
                            ...
                            <input id={`Update-datepicker-${this.update.id()}`} className="FormControl" type="text" required autocomplete="off" />
                        </div>
                        <div className="UpdateEditCard-item">
                        ...
                        <div className="UpdateEditCard-item UpdateEditCard-full">
                            ...
                            <a href={this.update.link()} target="_blank">
                                {this.update.link()}
                            </a>
                        </div>
                        <div className="UpdateEditCard-item UpdateEditCard-full">
                        ...
                        <div className="UpdateEditCard-item UpdateEditCard-full">
                            ...
                            <input type="textarea" bidi={this.updaterComment} />
                        </div>
                    </div>
    ...
    loadPlayer() {
        ...
        const preloadedplayer = app.store.all('players').filter((p) => p.user() && p.user().id() == this.update.submittedUser().id())[0];
        if (!!preloadedplayer) {
            this.loadingPlayer = false;
        ...
        app.store
            .find('users', `${this.update.submittedUser().id()}/player`, null, {
                errorHandler() {},
            })
            .then((p) => {
                this.player = p;
    ...
    approve() {
        ...
        this.update
            .save(this.data('approved'))
            .then(() => {
                app.alerts.show({ type: 'success' }, app.translator.trans('hcl.forum.alerts.approved'));
            })
            .finally(() => {
                this.saving = false;
            });
    ...
    deny() {
        ...
        if (!this.updaterComment()) {
            app.alerts.show({ type: 'error' }, app.translator.trans('hcl.forum.alerts.comment_required'));
            this.saving = false;
            return;
        }

        this.update
            .save(this.data('denied'))
            .then(() => {
                app.alerts.show({ type: 'success' }, app.translator.trans('hcl.forum.alerts.denied'));
            })
            .finally(() => {
                this.saving = false;
            });
    ...
    underReview() {
        ...
        this.update
            .save(this.data('under_review'))
            .then(() => {
                app.alerts.show({ type: 'success' }, app.translator.trans('hcl.forum.alerts.underReview'));
            })
            .finally(() => {
                this.saving = false;
            });
