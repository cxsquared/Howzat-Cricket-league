...
function roleOptions(bowlingStyle) {
    ...
    for (const role in mapping) {
        options[role] = app.translator.trans(`hcl.lib.player_role.${role}`, { style: app.translator.trans(`hcl.forum.player.style.${bowlingStyle}`) });
    }
...
function playerItem(player, gmView, changeRole) {
        ...
        if (gmView) {
            baseView.push(<div>
                <Select options={roleOptions(player.bowlingStyle())}
                        value={player.role() || 'batter'}
                        onchange={(v) => changeRole(player, v)} />
            </div>);
        } else {
            let role = player.role() || 'batter';
            baseView.push(<div>
                {app.translator.trans(`hcl.lib.player_role.${role}`, { style: app.translator.trans(`hcl.forum.player.style.${player.bowlingStyle()}`) })}
            </div>);
        }
            ...
            <div className="TeamCard" key={this.forcedRefreshKey} oncreate={this.onListCreate.bind(this)}>
                ...
                <div className="TeamCard-players">
                    ...
                    <ul className="PlayerList">
                        {this.getPlayers().map((p) => playerItem(p, this.gmView, this.changeRole.bind(this)))}
                    </ul>
                </div>
